...
-- Simplified version using only existing columns
-- ==========================================
...
SELECT
  ...
  -- SLA status
  CASE
    WHEN lead_score >= 75 AND EXTRACT(EPOCH FROM (NOW() - created_at)) / 3600 > 1 THEN 'OVERDUE'
    WHEN lead_score >= 55 AND EXTRACT(EPOCH FROM (NOW() - created_at)) / 3600 > 4 THEN 'OVERDUE'
    ELSE 'On Time'
  END as sla_status
